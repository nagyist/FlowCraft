...
## Version 6.2.1 — Finalize & View no longer fails with "Render failed" after iterating in the workbench

If you drafted a diagram in the workbench and clicked **Finalize & View** only to land on a "Render failed — Unable to render diagram, the generated mermaid syntax was invalid" panel (with a `Could not find a suitable point for the given distance` error in the console), this is the fix. The Mermaid source was always valid — the layout pass was breaking on a stale config, not the syntax.

### 🐛 Single Mermaid initializer, per-render theme via the `%%{init}%%` directive

- The workbench preview and the finalized viewer were each calling `mermaid.initialize()` on the same module-global singleton with a different theme (dark for the preview, light for the viewer). On client-side navigation from one to the other, the second `initialize()` mutated layout config mid-session — node sizes were measured under one config and edges routed under another, which is the documented trigger for dagre's "Could not find a suitable point for the given distance" on otherwise-valid diagrams.
- Both render paths now go through a shared `lib/mermaidClient.ts` that calls `mermaid.initialize()` exactly once with neutral defaults. Each call site passes its theme as render-scoped config, injected into the source via Mermaid's `%%{init}%%` directive, so the singleton is never re-mutated.
