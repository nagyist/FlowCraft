...
- The shared mermaid client (`lib/mermaidClient.ts`) now also `await`s `document.fonts.ready` before the first init and before each render. This isn't what caused this bug, but it forecloses a separate font-load race that would have surfaced eventually.
